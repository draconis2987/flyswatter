...
class Fly {
  ...
  void render(Canvas c) {
    if (isDead) {
      deadSprite.renderRect(c, flyPos.inflate(flyPos.width / 2));
    } else {
      flyingSprite[flyingSpriteIndex.toInt()].renderRect(c, flyPos.inflate(flyPos.width / 2));
  ...
  void update(double t) {
    if (isDead) {
      // make the fly fall
      flyPos = flyPos.translate(0, game.tileSize * 12 * t);
      if (flyPos.top > game.screenSize.height) {
        isOffScreen = true;
      }
    } else {
      // flap the wings
      ...
      Offset toTarget = targetLocation - Offset(flyPos.left, flyPos.top);
      if (stepDistance < toTarget.distance) {
        Offset stepToTarget = Offset.fromDirection(toTarget.direction, stepDistance);
        flyPos = flyPos.shift(stepToTarget);
      } else {
        flyPos = flyPos.shift(toTarget);
        setTargetLocation();
      }
